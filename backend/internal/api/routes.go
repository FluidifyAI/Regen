...
func SetupRoutes(router *gin.Engine, db *gorm.DB, cfg *config.Config, teamsSvc *services.TeamsService, samlMiddleware *samlsp.Middleware, hooks enterprise.Hooks, localAuth services.LocalAuthService, announcementsGetter func() []byte) {
	...
	v1 := router.Group("/api/v1")
	{
		...
		migrationsGroup := protected.Group("/migrations", middleware.RequireAdmin())
		{
			...
			migrationsGroup.POST("/opsgenie/preview", handlers.PreviewOpsgenieMigration(scheduleRepo, escalationPolicyRepo))
			migrationsGroup.POST("/opsgenie/import", handlers.ImportOpsgenieMigration(scheduleRepo, escalationPolicyRepo))
		}
	}
