...
func (c *anthropicClient) Complete(ctx context.Context, messages []Message) (string, Usage, error) {
	...
	if err != nil {
		return "", Usage{}, fmt.Errorf("anthropic: send request: %w", err)
	}
	defer resp.Body.Close()

	var result anthropicResponse
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return "", Usage{}, fmt.Errorf("anthropic: decode response (status %d): %w", resp.StatusCode, err)
	}
	if result.Error != nil {
		return "", Usage{}, fmt.Errorf("anthropic api error [%s]: %s", result.Error.Type, result.Error.Message)
	}
	if len(result.Content) == 0 {
		return "", Usage{}, fmt.Errorf("anthropic: empty content in response (status %d)", resp.StatusCode)
	}
	var usage Usage
	if result.Usage != nil {
		usage = Usage{
